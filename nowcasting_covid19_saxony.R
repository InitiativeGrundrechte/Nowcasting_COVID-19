...
first_date_to_query <- now - days(30) # max. 5000 Einträge als Rückgabe

data <- fromJSON(paste0('https://services7.arcgis.com/mOBPykOjAyBO2ZKk/arcgis/rest/services/RKI_COVID19/FeatureServer/0/query?where=%28neuerfall%3D0+or+neuerfall%3D1%29+and+idbundesland%3D14+and+refdatum%3C%3Emeldedatum+and+meldedatum>\'', format(first_date_to_query, "%Y-%m-%d"), '\'+and+meldedatum%3C\'', format(last_valid_date, "%Y-%m-%d"), '\'&objectIds=&time=&resultType=none&outFields=Refdatum%2CMeldedatum%2CAnzahlFall&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false&returnDistinctValues=false&cacheHint=false&orderByFields=Refdatum&groupByFieldsForStatistics=Refdatum&outStatistics=&having=&resultOffset=&resultRecordCount=&sqlFormat=none&f=pjson&token='))
